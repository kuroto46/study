	...
	$.fn.slidePlugin = function(options) {
    	return this.each(function() {
			...
			var
				...
				defaultPosition    = Math.floor(imgNum / 2),
			...
			$slideshowSlides.css({
				...
				//デフォルトの位置に移動（スライドの真ん中）
				'-webkit-transform':'translate(' + defaultPosition * slideWidth * -1 + 'px,0)',
			});
			...
			var indicatorImg       =  $('.indicator:last-child').css('background-image'),
				...
				$slideshowIndicators.find('a').eq(0).removeClass('active').css({ backgroundImage : indicatorImg });
				$slideshowIndicators.find('a').eq(defaultPosition +1).addClass('active').css({ backgroundImage : indicatorImgActive });

			/*+++++++++++++++++時間経過とスライドの変更の処理+++++++++++++++++*/
			//画像を時間経過に伴いスライドさせる関数
			function movingTime(){
				//現在のスライドが何番目のスライドなのか取得
				var slideNumber     = Math.floor(( $this.find('.slideshow-slides').offset().left / slideWidth) * -1 +3);
				//最後のスライドの場合xz
